...
describe('TokenController', () => {
  ...
  describe('pause trueUSD and wipe accounts', function () {
    it('TokenController can pause TrueUSD transfers', async function () {
      ...
      expect(await tokenProxy.implementation())
        .to.equal('0x9821722854922f004a9EE24d8aedF3D422745062')
    })
  ...
  describe('requestReclaimTrx', function () {
    ...
    it('reclaims ether', async function () {
      ...
      await controller.requestReclaimTrx()
    ...
    it('cannot be called by non-owner', async function () {
      await forceEther.destroyAndSend(token.address)
      await expect(controller.connect(otherWallet).requestReclaimTrx())
        .to.be.reverted
    })
  })
  ...
  describe('setCanBurn', () => {
    it('sets whether address can burn', async () => {
      await controller.connect(owner).setCanBurn(otherWallet.address, true)
      expect(await token.canBurn(otherWallet.address)).to.be.true
      await controller.connect(owner).setCanBurn(otherWallet.address, false)
